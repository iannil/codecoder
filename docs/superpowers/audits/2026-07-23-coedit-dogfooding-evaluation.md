...
2. **自主性受两头夹击:弱模型 + 缺自恢复。** 本次大量摩擦(整轮只探索不动手、大文件截断、谎报通过)源于 `deepseek-v4-flash` 偏弱;而 codecoder 默认值(max_tokens 4096、12 工具/turn、needs_fix 不自动重试)**放大**了模型弱点。换强模型 + 补自恢复循环可省掉约一半人工干预。
