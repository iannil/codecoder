...
CodeCoder 是一个**已落地**的自主 AI agent，使用 Rust 编写。仓库已有 Cargo 项目、28 个源模块(`src/`,含 `background.rs`、`trust.rs`、`retry.rs`、`review.rs`、`workgraph.rs`)、25 个内置工具、154 个测试(150 通过 + 4 个 `#[ignore]`:2 Docker e2e + L2 pty 冒烟 + L3 真实 LLM 冒烟),以及 21 份 ADR(`docs/adr/`)。`tests/` 下为黑盒行为验证分层(L1 默认;L2/L3 门控),见 `docs/testing/behavioral-validation.md`。架构总览见 `ARCHITECTURE.md`;领域术语以 `CONTEXT.md` 为准。
